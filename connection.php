...
class Connection {
  ...
  public function SendTransaction($merchantObj, $request, $method) {
	...
	//curl_setopt($this->curlObj, CURLOPT_SSLVERSION, 6); //Integer NOT string TLS v1.2
	//curl_setopt($this->curlObj, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_1);
    ...
    if ($merchantObj->GetDebug()) {
      ...
      curl_setopt($this->curlObj, CURLOPT_HEADER, FALSE);
      curl_setopt($this->curlObj, CURLINFO_HEADER_OUT, FALSE);		
	}
    ...
    if ($merchantObj->GetDebug()) {
	  ...
	  {
		//Concatinate both Header and Body message
